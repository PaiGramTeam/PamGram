...
def roleToName(shortname: str) -> str:
    ...
    return next(
        (value[0] for value in roles.values() for name in value if name == shortname),
        shortname,
    )
...
def weaponToId(name: str) -> int | None:
    ...
    return next(
        (
            ...
            if weaponToName(name) in value["name"]
        ),
        None,
    )
...
def roleToTag(role_name: str) -> List[str]:
    ...
    return next(
        (value for value in roles.values() if value[0] == role_name), [role_name]
    )
