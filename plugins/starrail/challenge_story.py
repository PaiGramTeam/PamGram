...
class ChallengeStoryPlugin(Plugin):
    ...
    async def command_start(self, update: Update, context: CallbackContext) -> None:
        ...
        self.log_user(
            update,
            logger.info,
            "[bold]虚构叙事挑战数据[/bold]请求: uid=%s previous=%s",
            uid,
            previous,
            extra={"markup": True},
        )
    ...
    def get_season_data_name(data: "HistoryDataChallengeStory"):
        ...
        honor = ""
        total_stars = data.story_data.total_stars
        extra_stars = getattr(data.story_data, "extra_star_num", 0) or 0
    ...
    async def get_challenge_story_history_floor(self, update: "Update", data_id: int):
        """渲染层数数据"""
        callback_query = update.callback_query
        ...
        images = await self.get_rendered_pic(abyss_data.story_data, abyss_data.group, data.user_id)
    ...
    async def get_challenge_story_history(self, update: "Update", _: "ContextTypes.DEFAULT_TYPE") -> None:
        ...
        result, user_id, uid = await get_challenge_story_history_callback(callback_query.data)
        if user.id != user_id:
        ...
        if result.startswith("p_"):
            ...
            return
        data_id = int(result)
        await self.get_challenge_story_history_floor(update, data_id)
