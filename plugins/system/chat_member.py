...
class ChatMember(Plugin):
    ...
    @handler.chat_member(
        chat_member_types=ChatMemberHandler.MY_CHAT_MEMBER, block=False
    )
    ...
    async def greet(self, user: User, chat: Chat, context: CallbackContext) -> None:
        ...
        if quit_status:
            try:
                await context.bot.send_message(chat.id, "彦卿不想进去！不是旅行者的邀请！")
            ...
            try:
                await context.bot.send_message(
                    chat.id, "感谢邀请小彦卿到本群！请使用 /help 查看咱已经学会的功能。"
                )
            except Forbidden as exc:
                logger.info("发送消息失败 %s", exc.message)
            except NetworkError as exc:
